...
IUSE="debug +libffi llvm-gcc multitarget ocaml test udis86 rcore"
...
src_configure() {
	...
	if use rcore; then
		CONF_FLAGS="${CONF_FLAGS} --enable-jit"
		if use debug; then
			CONF_FLAGS="${CONF_FLAGS} --disable-optimized --enable-debug-runtime --enable-assertions"
			einfo "Note: RCORE Version"
			einfo "Note: Compiling LLVM in debug mode will create huge and slow binaries"
		else
			CONF_FLAGS="${CONF_FLAGS} --enable-optimized --enable-assertions"
			einfo "Note: RCORE Version"
		fi
	else 
		CONF_FLAGS="${CONF_FLAGS} --enable-shared"
		if use debug; then
			CONF_FLAGS="${CONF_FLAGS} --disable-optimized --enable-debug-runtime"
			einfo "Note: Compiling LLVM in debug mode will create huge and slow binaries"
